# v2.6 - Agent View with Split Broker Info
...
if data is not None and not data.empty:
    ...
    if not df_final.empty:
        ...
        with top_r:
            ...
            with st.container(height=520):
                for day in days:
                    ...
                    fig_day.update_layout(height=150, margin=dict(t=5, b=35, l=0, r=10), yaxis_visible=False)
        ...
        # --- REGISTRO DETALLADO CON DOS COLUMNAS DE BROKER ---
        ...
        # Seleccionamos las columnas finales para la tabla
        final_table = df_log[['Date_Only', 'Start', 'Finished', 'num_str', 'Broker_Name', 'Talk', 'Idle After', 'Gap_Category', 'is_repeat']]
        final_table.columns = ['Date', 'Start', 'Finished', 'Phone Number', 'Broker Name', 'Talk', 'Idle After', 'Category', 'Repeated']

        def style_audit_final(row):
            styles = [''] * len(row)
            cols = list(final_table.columns)
            if row['Repeated']: styles[cols.index('Phone Number')] = 'color: #8b0000; font-weight: bold;'
            cat = row['Category']
